...
public class ChatChannelMessageCommand extends BukkitCommand{
			...
			for(ChatChannel cc : LLChat.getChannels()){
				commandMap.register(cc.getName(), 
						new ChatChannelMessageCommand(cc.getName(), 
													  ...
													  "",
													  Arrays.asList(cc.getLanguage().getRawISO())));
			}
	...
	protected ChatChannelMessageCommand(String name, String description, String usageMessage, List<String> aliases) {
	...
	public boolean execute(CommandSender sender, String label, String[] args) {
		if (args.length > 0){
			cc.sendMessage(String.join(" ", args), sender instanceof Player ? ((Player) sender).getDisplayName() : sender.getName());
		}
		return true;
	}
